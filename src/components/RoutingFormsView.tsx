...
const RoutingFormsView: React.FC<RoutingFormsViewProps> = ({ isDarkMode }) => {
  const routingForms = [
    {
      id: 1,
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className={`text-sm mt-1 transition-colors duration-300 ${isDarkMode ? 'text-[#818181]' : 'text-gray-600'}`}>
          Create forms to route visitors to the right event type.
        </p>
      </div>
      ...
      {routingForms.length > 0 ? (
        <div className="space-y-4">
          {routingForms.map((form) => (
            <div key={form.id} className={`p-6 rounded-lg border transition-all duration-200 hover:shadow-md ${isDarkMode ? 'bg-[#212124] border-[#818181]/20 hover:border-[#818181]/40' : 'bg-white border-gray-200 hover:border-gray-300'}`}>
              <div className="flex items-center justify-between">
