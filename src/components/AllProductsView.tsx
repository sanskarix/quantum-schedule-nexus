...
const AllProductsView: React.FC<AllProductsViewProps> = ({
  ...
  return <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        return <div key={product.id} className={`p-6 rounded-lg border transition-all duration-200 hover:shadow-lg hover:-translate-y-1 ${isDarkMode ? 'bg-[#212124] border-[#818181]/20 hover:border-[#818181]/40' : 'bg-white border-gray-200 hover:border-gray-300'}`}>
              <div className="flex items-start justify-between mb-4">
                ...
                <span className={`px-2 py-1 rounded-full text-xs font-medium ${product.status === 'Active' ? 'bg-green-100 text-green-700' : 'bg-blue-100 text-blue-700'}`}>
                  {product.status}
                </span>
              </div>
              ...
              <div className="space-y-3">
                ...
                <Button variant="outline" className="w-full transition-all duration-200 hover:shadow-sm" disabled={product.status === 'Active'}>
                  {product.status === 'Active' ? 'Currently Using' : <>
